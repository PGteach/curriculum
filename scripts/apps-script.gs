...
/**
 * How much of each mistake goes into the Mistakes cell.
 *
 *   "full"   question, what they chose, the right answer, the English
 *            explanation they were shown, and the Arabic one if the question
 *            has it
 *   "answers" question, what they chose, the right answer — the old behaviour
 *
 * "full" is the useful one when you are reading a single student's row: it
 * shows what they were told, not just what they got wrong. The cost is row
 * height, because a student with eight mistakes then carries forty-odd lines
 * in one cell. Note also that the explanation is the same for every student
 * who missed that question, so at class scale it repeats a lot.
 *
 * If the rows get too tall to scan, set this to "answers". Nothing else has
 * to change — the quiz sends the same payload either way.
 */
var MISTAKE_DETAIL = "full";
 ...
 *
 * The quiz has always sent the explanation the student was shown -- `why`
 * in English, and `ar` in Arabic where the question has one -- and this
 * function used to drop both. It kept the cell short, but it meant that
 * looking at a mistake in the sheet told you what the student picked and
 * not what they were then told, which is the half that says whether the
 * lesson landed.
 *
 * Both are optional on purpose. Most lecture 1 questions have no `ar`, and
 * a submission that sat in the browser's retry queue from before the field
 * existed has neither -- so each line is printed only if it arrived.
 */
function formatMistakes_(list) {
  if (!list || !list.length) return "none";
  return list.map(function (w, i) {
    var out = (i + 1) + ". " + w.question +
              "\n   chose: " + w.selected +
              "\n   correct: " + w.correct;
    if (MISTAKE_DETAIL === "full") {
      if (w.why) out += "\n   why: " + w.why;
      if (w.ar)  out += "\n   بالعربي: " + w.ar;
    }
    return out;
...
function testSubmission() {
  var out = doPost({ postData: { contents: JSON.stringify({
    ...
    wrongQuestions: [{
      ...
      /* second mistake carries an Arabic explanation, and a third has
         neither -- so one test row exercises all three shapes the
         Mistakes cell has to render */
      question: "Which one describes AR rather than VR?",
      selected: "Immerses the user in a virtual space generated by a computer",
      correct: "Overlays digital information on real-world images",
      why: "AR adds a digital layer on top of the real world.",
      ar: "AR بيضيف طبقة رقمية فوق الواقع. VR بيستبدل الواقع كله."
    }, {
      question: "A question queued before the explanation field existed",
